...
char *recordToXML(Record *r, int *bufSize)
{
    ...
    if (r->has_courses)
    {
        ...
        sz = strlen(buffer);
        if (r->courses & Course_IN1000)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1000\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1010)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1010\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1020)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1020\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1030)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1030\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1050)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1050\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1060)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1060\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1080)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1080\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1140)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1140\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1150)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1150\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1900)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1900\" />\n");
            sz = strlen(buffer);
        }
        if (r->courses & Course_IN1910)
        {
            snprintf(buffer + sz, BUFSZ - sz, "    <course=\"IN1910\" />\n");
            sz = strlen(buffer);
        }
        snprintf(buffer + sz, BUFSZ - sz, "  </courses>\n");
        sz = strlen(buffer);
